...
FREEZE_LAYERS  = 0                 # 0 = train everything, 11 = freeze backbone
...
# ── Checkpoint ──────────────────────────────────────────────────────────────
CKPT_DIR       = Path("base")       # checkpoint output folder
CKPT_INTERVAL  = 2000               # save every N steps
...
# ── Validation ──────────────────────────────────────────────────────────────
VAL_INTERVAL   = 250                # compute val loss every N steps
...
def main():
    ...
    train_loader = get_dataloader(
        ...
        worker_init_fn=worker_init_fn, max_labels=MAX_LABELS,
        scale_range=SCALE_RANGE, rotate_deg=ROTATE_DEG,
        hsv_h=HSV_H, hsv_s=HSV_S_RANGE, hsv_v=HSV_V_RANGE,
    )
